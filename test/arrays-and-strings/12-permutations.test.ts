...
} from "../../src/arrays-and-strings/12-permutations";
...
describe("checkPermutationsWithHash", () => {
  ...
  it("returns true for long strings that are permutations", () => {
    expect(checkPermutationsWithHash("thequickbrownfox", "xofnworbkciuqeht")).toBe(
      true
    );
  });

  it("returns false for long strings that are not permutations", () => {
    expect(checkPermutationsWithHash("thequickbrownfox", "jumpsovethefoxdog")).toBe(
      false
    );
  });
});
